  ...
  internal partial class Candles : Style
  {
    ...
    public bool PaintOptimized(Series[] series, Series seriesClose)
    {
      ...
      foreach (PriceStyleStandardModel.Values value in model.WickValues)
      {
        ...
        {
          groupDown.Children.Add(new RectangleGeometry
                                   {
                                     Rect = new Rect(new Point(value.CandleValue.X1, value.CandleValue.Y1),
                                                     new Point(value.CandleValue.X2, value.CandleValue.Y2)),
                                   });
          if (wick != null)
          {
            groupWicksDown.Children.Add(new LineGeometry
                                          {
        ...
        {
          groupUp.Children.Add(new RectangleGeometry
                                 {
                                   Rect = new Rect(new Point(value.CandleValue.X1, value.CandleValue.Y1),
                                     new Point(value.CandleValue.X2, value.CandleValue.Y2))
                                 });
          if (wick != null)
          {
            groupWicksUp.Children.Add(new LineGeometry
                                        {
                                          StartPoint = new Point(wick.X, wick.Y1),
                                          EndPoint = new Point(wick.X, wick.Y2),
                                        });
