		...
		private void ReCompressTicks()
		{
			foreach (KeyValuePair<string, List<TickEntry>> pair in _tickValues)
			{
				...
				foreach (TickEntry tickEntry in pair.Value)
				{
					...
					{
						if (open != null) AppendValue(open.Series.SeriesIndex, tickEntry._timeStamp, tickEntry._lastPrice);
						if (high != null) AppendValue(high.Series.SeriesIndex, tickEntry._timeStamp, tickEntry._lastPrice);
						if (low != null) AppendValue(low.Series.SeriesIndex, tickEntry._timeStamp, tickEntry._lastPrice);
						if (close != null) AppendValue(close.Series.SeriesIndex, tickEntry._timeStamp, tickEntry._lastPrice);
						if (volume != null) AppendValue(volume.Series.SeriesIndex, tickEntry._timeStamp, tickEntry._lastVolume);              
						continue;
					}
